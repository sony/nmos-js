...
myApp.config(['NgAdminConfigurationProvider', function (nga) {
    ...
    node.listView()
      ...
      .filters([
        ...
        nga.field('hostname')
           ...
           .pinned(false)
           .template('<div class="input-group"><input type="text" ng-model="value" placeholder="{{field._name.substr(0,1).toUpperCase() + field._name.substr(1)}}" class="form-control"></input><span class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span></div>')
      ]);
    ...
    device.listView()
      .fields([
        nga.field('label').label('Label').isDetailLink(true).sortable(false),
        nga.field('type', 'choice').sortable(false)
           .choices([
    ...
    device.showView().title('Device: {{entry.values.label}}').fields([
      ...
      nga.field('type', 'choice').sortable(false)
         .choices([
           { value: 'urn:x-nmos:device:generic', label: 'Generic'}, // NOTE: is this definitely correct?
         ]),
      nga.field('version')
    ]);
    ...
    source.listView().fields([
      nga.field('label').label('Label').isDetailLink(true).sortable(false),
      nga.field('format', 'choice').sortable(false)
         .choices([
    ...
    sender.listView().fields([
      nga.field('label').label('Label').isDetailLink(true).sortable(false),
      nga.field('transport', 'choice').sortable(false)
         .choices([
    ...
    sender.showView().title('Sender: {{entry.values.label}}').fields([
       ...
       nga.field('manifest_href').template('<a href="{{value}}">{{value}}</a>')
    ...
    receiver.showView().title('Receiver: {{entry.values.label}}').fields([
       ...
       nga.field('device_id', 'reference')
           ...
           .targetField(nga.field('label'))
           .label('Device'),
       nga.field('subscription.sender_id'), // TODO: add ref
       nga.field('tags', 'json'),
       nga.field('description'),
...
myApp.config(['RestangularProvider', function (RestangularProvider) {
    RestangularProvider.addFullRequestInterceptor(function(element, operation, what, url, headers, params) {
        if (operation == "getList") {
            ...
            // custom filters
            if (params._filters) {
                params['query.match_type'] = 'substr';
